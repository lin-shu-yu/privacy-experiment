...
# Date last modified: 11 DEC 2025
...
##### latinsquare funnction definition #####
...
##### counterbalancing ######
# seed corresponds to group of participants - ensures replicability
latinsquare(2, reps=3, seed=1) # partition order
latinsquare(6, reps=1, seed=1) # activity order

# show partition order in a clean matrix - the same as the 2x2 latin sq
partition_order <- matrix(nrow = 6, ncol = 3)
partition_order[,1] <- 0
partition_order[,2] <- latinsquare(2, reps=3, seed=1)[,1] 
partition_order[,3] <- latinsquare(2, reps=3, seed=1)[,2] 
...
for(i in 1:6){
  for(j in 1:3){
    block_1[i,j] <- intrusion_order[latinsquare(6, reps=1, seed=1)[i,j]]
  }
}
...
for(i in 1:6){
  for(j in 1:3){
    block_2[i,j] <- intrusion_order[latinsquare(6, reps=1, seed=2)[i,j]]
  }
}
...
for(i in 1:6){
  for(j in 1:3){
    block_3[i,j] <- intrusion_order[latinsquare(6, reps=1, seed=3)[i,j]]
  }
}
...
for(i in 1:6){
  for(j in 1:3){
    block_4[i,j] <- intrusion_order[latinsquare(6, reps=1, seed=4)[i,j]]
  }
}
...
for(i in 1:6){
  for(j in 1:3){
    block_5[i,j] <- intrusion_order[latinsquare(6, reps=1, seed=5)[i,j]]
  }
}
...
for(i in 1:6){
  for(j in 1:3){
    block_6[i,j] <- intrusion_order[latinsquare(6, reps=1, seed=6)[i,j]]
  }
}

##### inflatable knit #####
# from knit_inflatable_04
# create transfers in grid pattern
...
# every other column is whited out
pixels[:,edge+4:width-edge-5:2] = white
...
for k in channel_med:
    for j in range(int(k-channel_max/2), int(k+channel_max/2)):
        if (all(pixels[-1,j]==dark_green) and (j < k) and all(pixels[-2,j]==dark_blue)):
            pixels[-2,j] = yellow
        if (all(pixels[-1,j]==dark_green) and (j >= int(k)) and all(pixels[-2,j]==dark_blue)):
            pixels[-3,j] = yellow
...
final.save('knit_inflatable_04a.bmp')
final.save('knit_inflatable_04a.png')
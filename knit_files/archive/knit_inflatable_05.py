##### inflatable knit #####
# create dropped stitches with hex pattern

# Author: Mich Lin (kerberos: shuyulin)
# Date created: 17FEB2026
# Date last modified: FEB162026
...
opening = int(2); # make opening 3 st on each side
...
pixels[::2, 5:-5:4] = orange
pixels[1::2, 7:-6:4] = orange
...
# white out lines for dropped needles
knit_start[:,edge+4:width-edge-5:2] = white

# create edges
pixels[:,5:edge+5] = dark_blue; # left edge
pixels[:,width-edge-5:-5] = dark_blue; #right edge

# every other column is knitted until the end
pixels[:,edge+4:width-edge-5:2] = dark_blue
pixels[::25,edge+4:width-edge-5:2] = white
pixels[1::25,edge+4:width-edge-5:2] = dark_orange # transfer just below dropped stitches
...
pixels[-2, (left_opening_i-3):(left_opening_i+3):2] = yellow;
pixels[-3, (left_opening_i-3+1):(left_opening_i+3):2] = yellow;
# right edge
pixels[-2, (right_opening_i-3):(right_opening_i+3):2] = yellow;
pixels[-3, (right_opening_i-3+1):(right_opening_i+3):2] = yellow;
for i in range(num_channels):
    pixels[-2, (channel_med[i]-opening):(channel_med[i]+opening):2] = yellow;
    pixels[-3, (1+channel_med[i]-opening):(channel_med[i]+opening):2] = yellow;
...
final.save('knit_inflatable_05.bmp')
final.save('knit_inflatable_05.png')